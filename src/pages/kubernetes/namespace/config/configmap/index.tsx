import { AlertFilled, CloseCircleOutlined, DeleteOutlined, EditOutlined, EyeInvisibleOutlined, EyeOutlined, InsertRowBelowOutlined, MoreOutlined, UnorderedListOutlined } from '@ant-design/icons';
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/config/configmaps`),
                );
              }}
            >
      ...
      render: (dom, entity) => {
        ...
        return (
          <Space orientation='vertical'>
            <a
              onClick={() => {
                window.location.pathname = normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/config/configmaps/${entity?.metadata?.name}`);
              }}
            >
      ...
      render: (_, record) => {
        ...
        if (record?.metadata && record.metadata.name !== 'kube-root-ca.crt') {
          nodes.push(<a
            key="edit"
            onClick={() => {
              window.location.href = normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${record?.metadata?.namespace}/config/configmaps/${record?.metadata?.name}/update`);
            }}
          >
