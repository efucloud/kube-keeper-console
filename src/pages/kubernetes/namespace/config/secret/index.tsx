        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/config/secrets`),
                );
              }}
            >
      ...
      render: (dom, entity) => {
        ...
        return (
          <Space orientation='vertical'>
            <a
              onClick={() => {
                window.location.pathname = normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/config/secrets/${entity?.metadata?.name}`);
              }}
            >
          ...
          if (!(record.metadata?.ownerReferences && record.metadata?.ownerReferences?.length > 0)) {
            if (['Opaque', 'kubernetes.io/tls', 'IngressTLS', 'kubernetes.io/dockerconfigjson'].includes(record.type || '')) {
              nodes.push(<a
                key="edit"
                onClick={() => {
                  window.location.href = normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${record?.metadata?.namespace}/config/secrets/${record?.metadata?.name}/update`);
                }}
              >
