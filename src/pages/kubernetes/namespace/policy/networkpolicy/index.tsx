...
const IndexDashboard: React.FC = () => {
  ...
  const columns: ProColumns<INetworkPolicy>[] = [
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/policy/networkpolicy`),
                );
              }}
            >
    ...
    },
    {
      ...
      render: (dom, entity) => {
        return <a onClick={() => { window.location.href = normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/policy/networkpolicy/${entity?.metadata?.name}/update`); }}>{entity?.metadata?.name}</a>;
      },
    },
    {
          ...
          <Space separator={<Divider orientation="vertical" />}>
            ...
            <Access accessible={true} key={'create'}>
              <Button
                ...
                key="create"
                onClick={() => {
                  window.location.href = normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${namespace}/policy/networkpolicy/create/text`);
                }}
              >
