        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/workload/replicasets`),
                );
              }}
            >
          ...
          return (
            ...
            >
              <a
                onClick={() => {
                  if (namespace) {
                    window.location.pathname = normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/workload/replicasets/${entity?.metadata?.name}`);
                  } else {
                    window.open(
                      normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/workload/replicasets/${entity?.metadata?.name}`),
                      '_blank',
                    );
                  }
                }}
      ...
      render: (_, record) => {
        ...
        if (!record?.metadata?.ownerReferences) {
          nodes.unshift(
            <a
              onClick={() => {
                window.location.href = normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${record?.metadata?.namespace}/workload/replicasets/${record?.metadata?.name}/update`);
              }}
            >
