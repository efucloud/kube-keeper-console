        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/workload/daemonsets`),
                );
              }}
            >
            ...
            <>
              ...
              >
                <a
                  onClick={() => {
                    if (namespace) {
                      window.location.pathname = normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/workload/daemonsets/${entity?.metadata?.name}`);
                    } else {
                      window.open(
                        normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/workload/daemonsets/${entity?.metadata?.name}`),
                        '_blank',
                      );
                    }
                  }}
        ...
        if (!record?.metadata?.ownerReferences) {
          nodes.unshift(
            <a
              key="edit"
              onClick={() => {
                window.location.href = normalizeKubernetesPath(`/kubernetes/cluster/${cluster}/namespace/${record?.metadata?.namespace}/workload/daemonsets/${record?.metadata?.name}/update`);
              }}
            >
